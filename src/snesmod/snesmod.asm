...
; modified for 64tass by Matthew Laux
...
; ready to transfer
;----------------------------------------------------------------------
	lda	SNESMOD_SPC	; read first byte
	xba			;
	lda	#0		;
	ldx	#1		;
	bra	sb_start	;
...
-	cmp	REG_APUIO0	; wait for SPC
	...
	ina			; add 2 or so...
	ina			;--------------------------------------
				; mask data so invalid $80 message wont get sent
...
;----------------------------------------------------------------------
	...
	beq	_process_again		; changes Y times
	sta	spc1			;
	dey				;
	bne	_process_again		;
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------------
	...
	bmi	_use_default_pitch	; otherwise use direct	
	sta	digi_pitch		;
	bra	_direct_pitch		;
_use_default_pitch			;
	lda	[SoundTable], y		;
...
;----------------------------------------------------------------------------
	...
	bmi	_use_default_pan	; otherwise use direct
	sta	spc1
	bra	_direct_pan
_use_default_pan
	lda	[SoundTable], y
